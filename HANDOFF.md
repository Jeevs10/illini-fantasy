...
| Tests | 66 passing (`npm test`, needs local Postgres — see README) |
...
tip-off locking, and five screens (matchup, my team, player pool, player card,
standings).

Not done, in the order I would take them:

1. **Commissioner surface.** Invites are CLI-only (`npm run league -- invite`).
   There is no way to add a manager from the app, which makes the app unusable
   for its actual purpose without a terminal. `packages/league/src/membership.ts`
   already has `inviteToLeague`, `revokeInvite` and `openInvites`; it needs a
   route and a form.
2. **Draft room (Phase 4).** The hard deadline — the season tips in November and
   a draft has no second chance. Nothing exists yet.
3. **Waivers.** `roster_slot` and `transaction` model the claim/release
   lifecycle and `claimPlayer` / `releasePlayer` enforce it, but nothing
   schedules or resolves a FAAB bid.
4. **Draw the games cap** and the rest of the design backlog — see
